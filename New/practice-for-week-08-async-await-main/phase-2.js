...
async function workout(totalTime) {
  try {
  const exer1 = await stretch(totalTime)
  const exer2 = await runOnTreadmill(exer1)
  const exer3 = await liftWeights(exer2);
  console.log(`done working out with ${exer3/1000} seconds left`);
  } catch (err) {
